...
import wrteam.Omagric.app.activity.DrawerActivity;
import wrteam.Omagric.app.activity.LoginActivity;
import wrteam.Omagric.app.activity.MainActivity;
import wrteam.Omagric.app.helper.AndroidMultiPartEntity;
import wrteam.Omagric.app.helper.ApiConfig;
import wrteam.Omagric.app.helper.AppController;
...
import wrteam.Omagric.app.model.City;
import wrteam.Omagric.app.ui.CircleImageView;

import static android.app.Activity.RESULT_OK;
import static android.content.Context.INPUT_METHOD_SERVICE;
import static wrteam.Omagric.app.helper.ApiConfig.createJWT;
import static wrteam.Omagric.app.helper.ApiConfig.getAddress;
...
public class ProfileFragment extends Fragment implements OnMapReadyCallback {
    ...
    EditText edtname, edtemail, edtaddress, edtPinCode, edtMobile ,edtcity,edtState;
    Session session;
       ...
       // areaSpinner = root.findViewById(R.id.areaSpinner);
        ...
        btnsubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                else if (ApiConfig.CheckValidattion(name, false, false))
                    edtcity.setError(getString(R.string.select_city));
                else if (ApiConfig.CheckValidattion(name, false, false))
                    edtState.setError(getString(R.string.select_state));
...
//                    Toast.makeText(getContext(), getResources().getString(R.string.selectarea), Toast.LENGTH_LONG).show();
                    ...
                    params.put(Constant.CITY_ID, city);
                    params.put(Constant.AREA_ID, state);
                            ...
                            if (result) {
                                try {
                                    ...
                                    if (!objectbject.getBoolean(Constant.ERROR)) {
                                        ...
                                        session.setData(Session.KEY_AREA, state);
                                        session.setData(Session.KEY_MOBILE, mobile);
                                        session.setData(Session.KEY_CITY_ID, city);
                                        session.setData(Session.KEY_AREA_ID, state);
        ...
        edtcity.setText(session.getData(session.KEY_CITY_ID));
        edtState.setText(session.getData(session.KEY_AREA_ID));
...
//                        int pos = 0;
//                        if (!objectbject.getBoolean(Constant.ERROR)) {
//                            areaList.add(0, new City("0", getString(R.string.select_city)));
//                            JSONArray jsonArray = objectbject.getJSONArray(Constant.DATA);
//                            for (int i = 0; i < jsonArray.length(); i++) {
//                                JSONObject jsonObject = jsonArray.getJSONObject(i);
