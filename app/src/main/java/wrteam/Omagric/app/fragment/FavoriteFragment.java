...
import wrteam.Omagric.app.adapter.FavoriteLoadMoreAdapter;
import wrteam.Omagric.app.adapter.OfflineFavoriteAdapter;
...
public class FavoriteFragment extends Fragment {
            ...
            public void onRefresh() {
                if (AppController.isConnected(activity)) {
                    if (isLogin) {
                        ...
                        favoriteLoadMoreAdapter = null;
                        favoriteArrayList.clear();

                        GetData();
                    } else {
                        offlineFavoriteAdapter = null;
    ...
    private void GetData() {
        ...
        params.put(Constant.OFFSET, Constant.FAVORITE_OFFSET + "");
                                            ...
                                            if (favoriteArrayList.size() < total) {
                                                if (!isLoadMore) {
                                                    if (linearLayoutManager != null && linearLayoutManager.findLastCompletelyVisibleItemPosition() == favoriteArrayList.size() - 1) {
                                                        ...
                                                        Constant.FAVORITE_OFFSET = Constant.FAVORITE_OFFSET + Constant.LOAD_ITEM_LIMIT;
                                                        Map<String, String> params = new HashMap<>();
                                                        params.put(Constant.GET_FAVORITES, Constant.GetVal);
                                                        params.put(Constant.USER_ID, session.getData(Constant.ID));
                                                        params.put(Constant.LIMIT, "" + Constant.LOAD_ITEM_LIMIT);
